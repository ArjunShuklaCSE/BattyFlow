...
try{
 ...
 await page.click('#record');await page.waitForFunction(()=>document.querySelector('#state')?.textContent==='recording');
 const overlay=await inspect("(()=>{const e=process.getBuiltinModule('module').createRequire(process.resourcesPath+'/app.asar/package.json')('electron');const w=e.BrowserWindow.getAllWindows().find(w=>w.webContents.getURL().includes('overlay=1'));return {visible:w.isVisible(),focusable:w.isFocusable(),focused:w.isFocused()};})()");assert.deepEqual(overlay.result.value,{visible:true,focusable:false,focused:false});evidence.checks.push('Packaged recording overlay is visible and never takes focus');
 const overlayPage=browser.contexts().flatMap(c=>c.pages()).find(p=>p.url().includes('overlay=1'));await overlayPage.screenshot({path:resolve('.local/recording-overlay.png')});
 await page.waitForTimeout(4500);await page.click('#record');await page.waitForFunction(()=>['ready','error'].includes(document.querySelector('#state')?.textContent),null,{timeout:100000});
