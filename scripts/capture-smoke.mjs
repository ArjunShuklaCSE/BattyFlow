...
if(process.argv.includes('--portable')){await import('./portable-smoke.mjs');process.exit(0);}
...
const instance=await electron.launch({...(packaged?{executablePath:resolve(portable?'release/BattyFlow 0.1.0.exe':'release/win-unpacked/BattyFlow.exe')}:{}),args,env:{...process.env,BATTYFLOW_DATA_DIR:dataDir},timeout:30000});
